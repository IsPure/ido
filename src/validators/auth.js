...
//password
const password = check('password').isLength({ min: 6, max: 25}).withMessage('Password must be a minimum of 6 characters. Please try again.');
...
const loginFieldsCheck = check('email').custom(async (value, { req }) => {
    ...
    if(!user.rows.length) {
        throw new Error('The username/password you used was incorrect. Please Try again')
    }
